...
Loadng the source graphics file rather than exports currently must be done in either Blender or Inkscape, however, if you have a different editor for .svg graphics or 3d modeling on your machine you could try changing line number 27 in the interfaceStuff.py file. Index number 2 is a tuple for .svg editor and index number 3 is a tuple for 3d model editor: change: name, file-type, and icon (as a .png) and it should work with other editors, THIS HAS NOT BEEN TESTED ON ANYTHING EXCEPT BLENDER AND INKSCAPE.
